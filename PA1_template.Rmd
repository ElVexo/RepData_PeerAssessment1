...
Interval `r max_steps` has the most number of steps averaged across all the days in the dataset.
...
There are `r missing_values` missing values in the raw data.
...
The new mean total number of steps taken per day is `r new_mean_steps`.  
The new median total number of steps taken per day is `r new_median_steps`.  
Filling in missing data using this method **has** changed the mean and median step values.  
There are `r diff_mean_steps` fewer mean steps and `r diff_median_steps` fewer median steps.
